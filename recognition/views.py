...
def add_photos(request):
	...
	if request.method=='POST':
		...
		if username_present(username):
			...
			messages.success(request, f'Base de données sont crée')
			return redirect('add-photos')
		else:
			messages.warning(request, f'Aucun nom d utilisateur trouvé. Veuillez d abord enregistrer l employé.')
...
def mark_your_attendance(request):
	...
	while(True):
		...
		for face in faces:
			print("INFO : à l'intérieur de la boucle")
			(x,y,w,h) = face_utils.rect_to_bb(face)
		...
		#Showing the image in another window
		#Creates a window with window name "Face" and with the image img
		cv2.imshow("Marquer la présence - Entrer - Appuyez sur q pour quitter",frame)
		#Before closing it we need to give a wait command, otherwise the open cv wont work
		# @params with the millisecond of delay 1
		#cv2.waitKey(1)
		#To get out of the loop
...
def mark_your_attendance_out(request):
	...
	while(True):
		...
		#Showing the image in another window
		#Creates a window with window name "Face" and with the image img
		cv2.imshow("Marquer la présence - Sortir - Appuyez sur q pour quitter",frame)
		#Before closing it we need to give a wait command, otherwise the open cv wont work
		# @params with the millisecond of delay 1
		#cv2.waitKey(1)
		#To get out of the loop
...
def train(request):
	...
	messages.success(request, f'Entraînement terminée.')
	...
	if request.method=='POST':
		...
		if form.is_valid():
			...
			if(len(time_qs)>0 or len(present_qs)>0):
				...
				messages.warning(request, f'Aucun enregistrement pour la date sélectionnée.')
		...
		if form.is_valid():
			...
			if username_present(username):
				...
				if date_to < date_from:
					messages.warning(request, f'Sélection de date invalide.')
					...
					if (len(time_qs)>0 or len(present_qs)>0):
						...
						messages.warning(request, f'Aucun enregistrement pour la durée sélectionnée.')
						return redirect('view-attendance-employee')
				...
				messages.warning(request, f'Aucun nom d utilisateur trouvé.')
				return redirect('view-attendance-employee')
